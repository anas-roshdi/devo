...
class ManageProductsWindow:
    def __init__(self, root):
        self.root = root
        self.root.title(t('manage_products_title'))
    ...
    def create_widgets(self):
        ...
        # --- Main Title Section ---
        tk.Label(self.root, text=t('manage_products_header'), 
                 font=Fonts.HEADER_MEDIUM, fg=Colors.PRIMARY_DARK).pack(pady=15)

        # --- Top Section: Input/Edit Form Frame ---
        form_frame = tk.LabelFrame(self.root, text=t('product_info_frame'), 
                                   padx=15, pady=15, font=Fonts.LABEL_FRAME)
        form_frame.pack(padx=20, pady=10, fill="x")

        # Row 0: ID (ReadOnly) and Name Entry
        tk.Label(form_frame, text=t('lbl_product_id')).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ...
        tk.Label(form_frame, text=t('lbl_name')).grid(row=0, column=2, sticky="w", padx=5, pady=5)
        ...
        tk.Label(form_frame, text=t('lbl_category')).grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.ent_cat = tk.Entry(form_frame, width=20)
        self.ent_cat.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(form_frame, text=t('lbl_unit_price')).grid(row=1, column=2, sticky="w", padx=5, pady=5)
        ...
        tk.Label(form_frame, text=t('lbl_type')).grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ...
        tk.Label(form_frame, text=t('lbl_size')).grid(row=2, column=2, sticky="w", padx=5, pady=5)
        ...
        tk.Button(btn_frame, text=t('btn_add_product'), bg=Colors.GREEN_DARK, fg=Colors.TEXT_WHITE,
                  width=18, font=Fonts.BTN_MEDIUM, command=self.add_product).pack(side=get_pack_side(tk.LEFT), padx=10)
        
        # Button to update existing record details
        tk.Button(btn_frame, text=t('btn_update_product'), bg=Colors.BLUE_DARK, fg=Colors.TEXT_WHITE,
                  width=18, font=Fonts.BTN_MEDIUM, command=self.update_product).pack(side=get_pack_side(tk.LEFT), padx=10)
        
        # Button to delete a record permanently
        tk.Button(btn_frame, text=t('btn_delete_product'), bg=Colors.RED_DARK, fg=Colors.TEXT_WHITE,
                  width=18, font=Fonts.BTN_MEDIUM, command=self.delete_product).pack(side=get_pack_side(tk.LEFT), padx=10)
        
        # Button to clear all entry fields
        tk.Button(btn_frame, text=t('btn_clear_fields'), bg=Colors.GRAY, fg=Colors.TEXT_WHITE,
                  width=18, font=Fonts.BTN_MEDIUM, command=self.clear_fields).pack(side=get_pack_side(tk.LEFT), padx=10)

        # --- Bottom Section: Table View ---
        tk.Label(self.root, text=t('lbl_inventory_list'), font=Fonts.BODY_ITALIC).pack(pady=(10, 0))

        table_frame = tk.Frame(self.root)
        table_frame.pack(padx=20, pady=10, fill="both", expand=True)

        # Define table columns
        columns = (t('col_id'), t('col_name'), t('col_category'), t('col_price'), t('col_type'), t('col_size'))
